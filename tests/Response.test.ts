...
import { makeFakeAPIGatewayEvent } from './fakes/api-gateway-event';
...
describe('Response', () => {
   ...
   beforeEach(() => {
      app = new Application();
      sampleReq = new Request(app, makeFakeAPIGatewayEvent(), handlerContext());
   ...
   describe('header functionality', () => {
      ...
      it('logs an error when passed two args and the second is not a string', () => {
         ...
         const val: any = true,
               req = new Request(app, makeFakeAPIGatewayEvent(), handlerContext()),
      ...
      const test = (referer: string | null | false, expectation?: string): void => {
         it(`sets the Location header - special value 'back' - referer '${referer}'`, () => {
            ...
            if (referer === null) {
               ...
               evt = _.extend(makeFakeAPIGatewayEvent(), {
   ...
   describe('body property', () => {
      it('contains what was sent in the response', () => {
         const resp = new Response(app, new Request(app, makeFakeAPIGatewayEvent(), handlerContext()), spy());
   ...
   describe('response sending functions', () => {
      ...
      describe('end', () => {
         ...
         it('sends immediately - APIGW', () => { test(makeFakeAPIGatewayEvent(), false, false); });
         it('sends immediately - ALB', () => { test(albRequest(), 'OK', false); });
         it('sends immediately - ALB MV', () => { test(albMultiValHeadersRequest(), 'OK', false); });
         it('sends after setting headers - APIGW', () => { test(makeFakeAPIGatewayEvent(), false, true); });
      ...
      describe('json', () => {
         ...
         it('sends immediately - ALB', () => { test(albRequest(), 'OK', false); });
         it('sends immediately - ALB MV', () => { test(albMultiValHeadersRequest(), 'OK', false); });
         it('sends after headers - APIGW', () => { test(makeFakeAPIGatewayEvent(), false, true); });
         ...
         it('sends with alternate status code - APIGW', () => { test(makeFakeAPIGatewayEvent(), false, false, ext1); });
         ...
         it('overrides previously-set content type - APIGW', () => { test(makeFakeAPIGatewayEvent(), false, false, ext2); });
      ...
      describe('jsonp', () => {
         ...
         it('sends immediately - APIGW', () => { test(makeFakeAPIGatewayEvent(), false); });
         ...
         it('sends after headers - APIGW', () => { test(makeFakeAPIGatewayEvent(), false, ext1); });
         ...
         it('works with custom callback param name - APIGW', () => {
            test(makeFakeAPIGatewayEvent(), false, ext2, { queryParamName: 'cbFnName' });
         });
         ...
         it('works like JSON when no callback in query - APIGW', () => {
            test(makeFakeAPIGatewayEvent(), false, expectJSON, { queryParamName: false });
         });
         ...
         it('works like JSON when non-callback param is in query - APIGW', () => {
            test(makeFakeAPIGatewayEvent(), false, expectJSON, { queryParamName: 'notcallback' });
         });
         ...
         it('uses the first callback param value listed - APIGW', () => {
            test(makeFakeAPIGatewayEvent(), false, undefined, {
         ...
         it('returns JSON when callback param value contains invalid characters - APIGW', () => {
            test(makeFakeAPIGatewayEvent(), false, expectJSON, {
         ...
         it('escapes UTF newline and paragraph separators - APIGW', () => {
            test(makeFakeAPIGatewayEvent(), false, ext3, { responseObject: utfInput });
         });
      ...
      describe('send', () => {
         ...
         it('sends string immediately - APIGW', () => { test(makeFakeAPIGatewayEvent(), 200, false, 'body', ext1); });
         ...
         it('sends string with existing content type - APIGW', () => { test(makeFakeAPIGatewayEvent(), 200, false, 'body', ext2); });
         ...
         it('sends JSON immediately - APIGW', () => { test(makeFakeAPIGatewayEvent(), 200, false, bodyObj, ext3); });
      ...
      describe('sendStatus', () => {
         ...
         _.each({ 'OK': 200, 'Created': 201, 'Not Found': 404, 'I\'m a teapot': 418 }, (code, msg) => {
            it(`sends immediately - APIGW - ${code} ${msg}`, () => { test(makeFakeAPIGatewayEvent(), code, false, false); });
            ...
            it(`sends after headers - APIGW - ${code} ${msg}`, () => { test(makeFakeAPIGatewayEvent(), code, false, true); });
