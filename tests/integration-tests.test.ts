...
describe('integration tests', () => {
   ...
   const testOutcome = (method: string, path: string, expectedBody: string): void => {
      const cb = spy(),
            evt = makeFakeAPIGatewayEvent({ path, httpMethod: method });
   ...
   describe('very simple smoke test that hits a variety of touch points', () => {
      ...
      it('works - APIGW', () => {
         const cb = test(makeFakeAPIGatewayEvent({ path: '/hello/world' }));
   ...
   describe('request object', () => {
      ...
      it('has an immutable context property', () => {
         let evt = makeFakeAPIGatewayEvent({ path: '/test', httpMethod: 'GET' }),
